...
tags = db.Table('tags',db.Column('term_id',db.Integer, db.ForeignKey('search.id')),db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')))
...
class PersonalGifCollection(db.Model):
    ...
    gifs = db.relationship('Gif', secondary=user_collection,
                               backref=db.backref('personalCollections', lazy='dynamic'), lazy='dynamic')
...
# TODO 364: The following forms for searching for gifs and creating collections are provided and should not be edited.
# TODO: You SHOULD examine them so you understand what data they pass along and can investigate as you build your view functions in TODOs below.
...
def get_or_create_search_term(term):
    ...
    if searchTerm:
        ...
        gifs_list = get_gifs_from_giphy(term)
        for gif in gifs_list:
            gif = get_or_create_gif(title=gif['title'],url=gif['embed_url'])
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    # In other words, based on the input to this function,
    # if there exists a collection with the input name, associated with the current user,
    ...
    # and each Gif in the gif_list input should be appended to it (remember,
    # there exists a many to many relationship between Gifs and PersonalGifCollections).
...
def index():
    ...
    if request.method == "POST":
        ...
        print ('function output:',get_or_create_search_term(search_term))
        print("Search term in index:", search_term)
    ...
    # invoke get_or_create_search_term on the form input and redirect to the function corresponding to
    # the path /gifs_searched/<search_term> in order to see the results of the gif search. (Just a couple lines of code!)

    # HINT: invoking url_for with a named argument will send additional data.
    # e.g. url_for('artist_info',artist='solange') would send the data 'solange' to a route /artist_info/<artist>
    return render_template('index.html',form=form)

# Provided
@app.route('/gifs_searched/<search_term>')
def search_results(search_term):
    print("Search term in search_results:", search_term)
...
def search_terms():
    data = SearchTerm.query.all()
    return render_template('search_terms.html',all_terms=data)



    # TODO 364: Edit this view function so it renders search_terms.html.
    # That template should show a list of all the search terms that have been searched so far.
    # Each one should link to the gifs that resulted from that search.
    # HINT: All you have to do is make the right query in this view function and send the right data to the template!
    # You can complete this in two lines. Check out the template for more hints!
...
def create_collection():
    ...
    if form.validate_on_submit():
        ...
        data = get_or_create_collection(current_user=current_user,name=form.name.data,gif_list=gif_obj)
        return redirect(url_for('collections',collections=data))
...
def collections():
    ...
    data = PersonalGifCollection.query.filter_by(user_id=current_user.id).all()
